...
def index_events(db_path: Path) -> bool:
...
def run(db_path: Path, do_index: bool = False, query: str | None = None) -> bool:
    """Main entry point."""
    if do_index:
        return index_events(db_path)
    if query:
        return search(query, db_path)
    # Default: index
    return index_events(db_path)
